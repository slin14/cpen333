...
def producer() -> None:
    ...
    for _ in range(SIZE * 2): #we just produce twice the buffer size for testing
        ...
        empty.acquire()     # acquire (decrement) empty semaphore
        mutex.acquire()     # acquire lock
        buffer.insert(item) # critical section
        mutex.release()     # release lock
        full.release()      # release (increment) full semaphore
...
def consumer() -> None:
    ...
    for _ in range(SIZE * 2): #we just consume twice the buffer size for testing
        ...
        full.acquire()         # acquire (decrement) full semaphore
        mutex.acquire()        # acquire lock
        item = buffer.remove() # critical section
        mutex.release()        # release lock
        empty.release()        # release (increment) empty semaphore
